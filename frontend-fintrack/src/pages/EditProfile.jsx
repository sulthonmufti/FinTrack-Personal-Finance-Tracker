...
import { User, Mail, Save, Lock, ShieldCheck, Loader2 } from "lucide-react";
...
export default function Profile() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const response = await api.put('/auth/update-profile', { username, email });
        ...
        {activeTab === 'info' ? (
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text"
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-sm"
                    value={username}
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full md:w-fit px-8 py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70"
            >
